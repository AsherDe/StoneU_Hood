...
class TimetableParser {
  ...
  static List<CalendarEvent> parseTimetable(String htmlContent) {
    ...
    for (int rowIndex = 1; rowIndex < rows.length; rowIndex++) {
      ...
      for (int dayIndex = 0; dayIndex < cells.length && dayIndex < 7; dayIndex++) {
        ...
        for (final courseDiv in courseDivs) {
          // 跳过空单元格
          if (courseDiv.text.trim().isEmpty || courseDiv.text.trim() == '&nbsp;') {
            continue;
          }
  ...
  /// 解析课程单元格并提取课程信息
  static List<CalendarEvent> _parseCourseCell(dom.Element courseDiv, int dayOfWeek, TimeSlot timeSlot) {
    List<CalendarEvent> events = [];
    ...
    for (int i = 0; i < fontElements.length; i++) {
      ...
      if (i == 0 || title == '课程') {
        ...
        instructor = text;
      } else if (title.contains('周次') || text.contains('周') && text.contains('[')) {
  ...
  static List<WeekRange> _parseWeekRanges(String text) {
    ...
    final weekPattern = RegExp(r'(\d+(?:-\d+)?(?:,\d+(?:-\d+)?)*)\s*(?:\(周\)|\(周\)|\(周\))');
  ...
  static CalendarEvent _createEvent(
    ...
    // 获取当前学期第一天的日期
    final DateTime semesterStart = DateTime.now().subtract(Duration(days: DateTime.now().weekday - 1));
    
    // 计算这个特定课程的日期
    final eventDate = semesterStart.add(Duration(days: (week - 1) * 7 + dayOfWeek - 1));
